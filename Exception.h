/*
 * 父类：Exception
 * 子类：ArithmeticException,NullPointerException,IndexOutOfBoundsException,NoEnoughMemoryException,InvalidParameterException
 * 成员：m_message:信息; m_location: 位置;因为不知道这些信息的生命周期，所以要先复制出来。
 * 纯虚函数析构函数：不管是否纯虚，都要实现；有纯虚函数就是抽象类，抽象类没有对象；
 * __LINE__：行号；__FILE__：文件名；
 * 字符串默认为const char* 类型
*/
